...
namespace BlazorTable.Tests
{
    public class AddNullChecks
    {
        ...
        public void AddToSingle()
        {
            ...
            Expression<Action<string>> expression = _ => Console.WriteLine("Goody");
            var good = expression.Body;
            good.ToString().ShouldNotBeNull();
        }
        ...
        public void NonNullable2()
        {
            ...
            Expression<Func<Parent, object>> expression = x => x.Child.Name;
            var field = expression.Body.CreateNullChecks();
            field.ToString().ShouldBe("(x.Child != null)");
        }
